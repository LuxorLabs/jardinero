...
export function logReviewSegments(sandboxRunId: string, task: SandboxTask): PromptSegment[] {
  return [
    ...
    segment(EDITABLE_PROMPT_SEGMENT, 'Guidance', true, [
      'Do not treat a run as reviewed unless you successfully query the telemetry of the target the payload names: the clusters in its `clusters` list, and its namespace when it has one. That target is the environment under review, whether it runs production or staging traffic.',
      'An environment may live on any one of those clusters, so a cluster with no streams is ordinary and not a reason to stop: query the others before deciding. Report telemetry access as blocked only when none of the listed clusters answers.',
      '',
      'Phase 1 - Triage:',
