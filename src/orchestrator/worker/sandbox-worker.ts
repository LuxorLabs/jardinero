...
// loop can be exercised without a live sandbox.
...
export class SandboxWorkerRunner implements SandboxRunner {
  ...
  async run(context: SandboxRunContext): Promise<WorkerResult> {
    ...
    if (missingEnv.length > 0) {
      throw new Error(
        `${this.provider.name} runner is missing required environment variables: ${missingEnv.join(', ')}`,
      );
    }
    ...
    const trackSession = (nextSession: SandboxSession): void => {
      session = nextSession;
  ...
  private repoDocsAccess(session: SandboxSession, repoDir: string): RepoDocsAccess {
  ...
  private async ensureDockerSocketAccess(
    ...
    try {
      ...
      const status = parseDockerSocketStatus(execStdout(result));
      if (status === 'granted') {
        await context.publishEvent({
  ...
  private async configureGitIdentity(session: SandboxSession, repoDir: string): Promise<void> {
  ...
  private async configureGitHubCredentials(session: SandboxSession): Promise<void> {
    const result = await this.execRequired(
      session,
  ...
  private async writeCodexConfig(session: SandboxSession): Promise<void> {
  ...
  private async prepareCodexAuth(session: SandboxSession): Promise<void> {
...
function retryableSandboxSessionStartReason(error: unknown): string | undefined {
...
function commandOutput(result: SandboxExecResult): string {
  return [execStdout(result), execStderr(result)]
...
function normalizeCodexResult(
  ...
  const stderr = execStderr(result);
...
async function readTextFile(session: SandboxSession, path: string): Promise<string | undefined> {
