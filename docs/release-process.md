...
- **Both `linux/amd64` and `linux/arm64`**, so the tag resolves on Apple silicon as well as on a cluster node. A tag is an index over the two; the signature and provenance are made against that index, which is what a `docker pull` of the tag resolves to.
